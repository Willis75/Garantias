...
else
{
    ...
    $sql1="UPDATE canalizacion SET descripcion=?, proveedor=?, linea=?, periodo=?, recepcion=?, canalizado=?, dictaminador=?, observacion=? WHERE codigo=?";
    
    $result = $mysqli->prepare($sql1);
    $result->bind_param('sssssssss', $descripcion, $proveedor, $linea, $periodo, $recepcion, $canalizado, $dictaminador, $observacion, $codigo);
    $result -> execute();
