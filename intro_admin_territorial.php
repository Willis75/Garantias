...
if(!isset($_SESSION['user_id'])||$_SESSION['territorial']!=1)
{
    header("Location:logout.php");
}

$territorio = $_SESSION['territorio'];
            ...
            <div class="col-sm-12 columna">
        		...
        		<div id="tickets_abiertos">
        		    ...
        		    include 'conect_gar.php';
                                ...
                                $sql="SELECT * , DATEDIFF(CURDATE(),s.fecha) difhoy, DATEDIFF(s.fecha_cierre, s.fecha) datedif ,s.file_ruta FROM solic s WHERE sucursal in ($territorio) ORDER BY s.folio DESC";
                                ...
                                if (!$resultado) {
        			             printf("error: %s\n", $mysqli->error);
                                    ...
                                    ,"<tr><th>Folio</h><th>Sucursal</th><th>Estatus</th><th>Cliente</th><th>Línea</th><th>Canalización</th><th>Inicio</th><th>Revisión Sucursal</th><th>Fecha dictamen</th><th>Entrega dictamen</th><th>Entrega producto</th><th>Procede</th><th>Aplica</th><th>Cierre</th><th>Dias</th><th>Adjunto</th><th>Producto</th><th>Comentario</th><th>#_Serie</th><th>Causa falla</th></tr>";
        			                 ...
        			                 while($fila = $resultado->fetch_assoc()){
                                        ...
                                        if ($_GET['estatus_folio']==99) {
                                            echo "<tr><td><a href='coment_all_territorial.php?folio=".$fila['folio']."'>".$fila['folio']."</a></td> <td>".$sucursales[$fila['sucursal']]."</td> <td>".$estatus_b."</td> <td class='left'>".$fila['CUSTOMER_NAME']."</td> <td>".$fila['linea']."</td> <td>".utf8_encode($fila['canalizacion'])."</td> <td>".$fecha_inicio."</td> <td>".$fecha_revision."</td> <td>".$fecha_dictamen."</td> <td>".$entrega_dictamen."</td> <td>".$entrega_producto."</td> <td>".$procede."</td> <td>".$fila['aplica']."</td> <td>".$fecha_cierre."</td> <td>".$datedif."</td><td><a href='".$fila['file_ruta']."' target='_blank'>".$adjunto."</a></td><td>".$fila['PRODUCTO']."</td><td>".utf8_encode($fila['comentario_aplica'])."</td><td>".$fila['serie']."</td><td>".utf8_encode($fila['causa_falla'])."</td> <tr>";
                                        }
