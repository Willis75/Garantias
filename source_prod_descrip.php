...
if (isset($_GET['term'])){
    ...
    try {
        ...
        $stmt = $conn->prepare('SELECT * FROM productos WHERE DESCRIPCION LIKE :term ORDER BY DESCRIPCION');
        $stmt->execute(array('term' => '%'.$_GET['term'].'%'));
        
        while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            
                $row_array['label'] = utf8_encode($row['DESCRIPCION']);
                $row_array['value'] = utf8_encode($row['DESCRIPCION']);
